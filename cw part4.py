...
#Declaring VARIABLES
Total = 0
credit_range=[0,20,40,60,80,100,120] #LIST OF CREDIT RANGE
...
def file_handle(progression): # CREATING USER DEFINED FUNCTIONS 
    '''Opens a file and writes all the inputs of the progressions and displays the inputs'''
    f=open("textfile.txt","w")
    f.write(f"{progression} -  {PASS[j]} , {DEFER[j]} , {FAIL[j]}")
    f.close
    f=open("textfile.txt","r")
    print(f.read())
    f.close
#INTRO MESSAGE FOR THE STAFF VERSION
print("Staff Version with Histogram")

while True : #WHILE LOOP FOR REPETITION
    try: #EXCEPTION HANDLING FOR VALUE ERROR
        ...
        if Total != 120: #ERROR MESSAGE FOR INCORRECT TOTAL OF CREDITS
            ...
            print("Progress")
            Progress.append("*")
        elif Pas_credits == 100: #CONDITION TO TRAILER
        ...
        if choice.lower() == "q":
            ...
            for j in range(len(PASS)): #FOR LOOP TO PRINT THE SPECIFIC VALUES FROM THE LIST'S INDEX IN ORDER
                if PASS[j] == 120:
                    file_handle("Progress")
                elif PASS[j] == 100:
                    file_handle("Trailer")
                elif PASS[j]<= 80 and FAIL[j] <= 60:
                    file_handle("Module Retriever")
                elif FAIL[j] >=80:
                    file_handle("Excluded")
            print("")
